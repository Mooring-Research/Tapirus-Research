...
MT_cov<- MT_cov[,which(names(MT_cov) %in% c("Elev", "HFI", "d.Road", "Precip", "NDVI", "AvgMinTemp", "AvgMaxTemp", "NPP"))]

##Scaling
colsToScale<- which(names(MT_cov) %in% c("Elev", "d.Road", "Precip", "NDVI", "AvgMinTemp", "AvgMaxTemp", "NPP"))
MT_cov[colsToScale]<- scale(MT_cov[,colsToScale])

#####Model-Prep######################
#Double check that all modeling columns are scaled correctly 
MT_umf<- unmarkedFrameOccu(y=MT_tapir[,-1], siteCovs= MT_cov, obsCovs=list(Eff=MT_eff[,-1]))
...
MT_m.psiPrecipElevHFI.pEff           		  <- occu(~Eff~ Precip + HFI + Elev, MT_umf)
...
sink("Multivariate Model summaries/MT_multiMods3.txt", append = FALSE)

cat("24, July 2023\n")
cat("\n***Correlation Matrix***\n")
cor(MT_cov[, which(names(MT_cov) %in% c("Precip", "Elev", "d.Road", "NDVI", "AvgMinTemp", "AvgMaxTemp", "NPP" ))]) #correlation MTtrix on numerical fields  #"Precip"  "Elev"  "d.Road"  "NDVI" "AvgMinTemp" "AvgMaxTemp" "NPP" 
