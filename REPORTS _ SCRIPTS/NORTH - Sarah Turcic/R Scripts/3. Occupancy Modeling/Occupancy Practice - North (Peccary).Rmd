---
title: "Occupancy Model-North Data (Peccary)"
author: "Sarah Turcic"
date: "6/8/2022"
...
Step 1
```{r}
## read in peccary data
peccary <-read.csv("Peccary_Detection.csv")
head(peccary)
```
...
obs<- read.csv("Peccary_Detection.csv", row.names="X")
class(obs)
...
plogis(5.712896) ##occupancy probability
plogis(-5.02894	)  ##detection probability
...
#Note which models are better than the null (mod.prod, mod.road, mod.DisCore)
#use these models in your multivariate models, but disregard other variables
```

```{r}
#multivariate models including mod.prod, mod.road, mod.DisCore
mod.road.prod <- occu(~1~Prod+Road,umf)
mod.road.dis <- occu(~1~Road+DisCore,umf)
mod.prod.dis <- occu(~1~DisCore+Prod, umf)
mod.prod.dis.road <- occu(~1~Prod+DisCore+Road,umf)

#list models and compare AIC (including univariate models)
MultiList <- fitList(mod0, mod.prod, mod.road, mod.DisCore, mod.road.prod, mod.road.dis, mod.prod.dis, mod.prod.dis.road)
...
#best model is mod.road
summary(mod.road) #no c
summary(mod.road.prod) #no c
summary(mod.road.dis) #no c
summary(mod.prod.dis.road) #no c
summary(mod.prod)
summary(mod.prod.dis) #no c
summary(mod.DisCore) #no c
...
detList.peccary<-fitList(mod.Eff.forest.HFI,mod.forest.HFI,mod.elev.HFI,mod.Eff.elev.HFI,mod.forest.elev.HFI,mod.Eff.forest.elev.HFI,mod.Eff,mod.forest,mod.elev,mod.Eff.elev,mod.Eff.forest,mod.HFI,mod.Eff.HFI,mod.forest.elev,mod.Eff.forest.elev)
Peccary_AIC_Table<-modSel(detList.peccary)
str(Peccary_AIC_Table)
ModelRankPeccary<-as.data.frame(Peccary_AIC_Table@Full)
write.csv(ModelRankPeccary, "Peccary_AIC_Table_North.csv")
```


```{r}
#all models are SUPER close
##look at output for best model (elev)
#summary(mod.Eff.elev)
#plogis(7.205471) #occupancy prob
#plogis(-3.093067	+ -0.314945*obsCovs$eff) #detection prob - not sure how to do this
```
```{r}
#unscaled data (NaNs produced)
summary(mod.Eff)
summary(mod.forest)
summary(mod.elev)
...
summary(mod.Eff.elev) #significant! (yes NaNs)
summary(mod.forest.HFI) #significant! (no NaNs) - model did not converge

#scaled data (no significance)
summary(mod.eff.road.elev.HFI) #no c
summary(mod.road.elev.HFI) #no c
summary(mod.road.HFI.elev.prod) #no c
summary(mod.river.HFI.elev.road) #no c
summary(mod.eff.road.forest.elev.HFI)
summary(mod.road.elev) #no c
summary(mod.road.forest.elev.HFI) #no c
```
...
new.road<-data.frame(Road=seq(min(scale(siteCovs$Road)), max(scale(siteCovs$Road)), 0.0001))


#if you get an error saying object x not found, ensure you are using the same variable name
pred.psi<-predict(mod.road, newdata=new.road, "state") #ensure named the same if you get error

## Plot predicted occupancy against new values for elevation
plot(new.road$Road, pred.psi$Predicted, type="l", 
     xlab="Distance to Road", ylab="Occupancy probability", main = "How Distance to Road Impacts Peccary Occupancy Probability - North", ylim=c(0,1))

##add confidence intervals
##lty stands for line type, 2 codes a dashed line
points(new.road$Road,pred.psi$lower, type="l", lty=2, col="blue")
points(new.road$Road,pred.psi$upper, type="l", lty=2, col="blue")
```
```{r}
