...
summary(mod0)
plogis(0.231)  	# Probability of occupancy = 0.5574946
plogis(-0.705)	# Probability of detection = 0.3307046
...
#scale siteCovs
umf<-unmarkedFrameOccu(y=puma, siteCovs = as.data.frame(scale(siteCovs)), obsCovs=list(Eff=eff))
summary(umf)
...
#Note which models are better than the null (mod.prod, mod.HFI, mod.elev)
#use these models in your multivariate models, but disregard other variables

#multivariate models including mod.prod, mod.HFI, mod.elev
mod.NPP.HFI <- occu(~1~NPP_value+HFI,umf)
mod.NPP.elev <- occu(~1~NPP_value+elev,umf)
mod.NPP <- occu(~1~NPP_value,umf)

mod.HFI.elev <- occu(~1~HFI+elev,umf)
mod.HFI <- occu(~1~HFI,umf)

mod.elev <- occu(~1~elev, umf)

#list models and compare AIC (including univariate models)
MultiList <- fitList(mod0,mod.NPP.HFI,mod.NPP.elev, mod.NPP, mod.HFI.elev,mod.HFI, mod.elev)

#model selection
modSel(MultiList) #mod.HFI.elev, mod.NPP.elev very close


#now check models for significance
#models are ordered based on AIC

summary(mod.HFI.elev) #no c
summary(mod.NPP.elev) #no c
summary(mod.NPP.HFI)  
summary(mod.NPP)
summary(mod.HFI) #significant at 0.0399
summary(mod.elev)
summary(mod0)

#only mod.HFI is significant
