...
umf_north<- unmarkedFrameOccu(y=paca, siteCovs = , obsCovs=list(Eff=eff))

mod0 <- occu(~1~1, umf_north)  # Null Model
summary(mod0)
plogis(-0.013)  	# Probability of occupancy = 0.49675
plogis(0.171)	# Probability of detection = 0.5426461
...
#scale siteCovs
umf<-unmarkedFrameOccu(y=paca, siteCovs = as.data.frame(scale(siteCovs)), obsCovs=list(Eff=eff))
summary(umf)
...
modSel(UniList) #best model is mod.prod

#Note which models are better than the null (mod.HFI, mod.prod, mod.water)
#use these models in your multivariate models, but disregard other variables

#multivariate models including mod.HFI, mod.prod, mod.water
mod.NPP.HFI <- occu(~1~NPP_value+HFI,umf)
mod.NPP.water <- occu(~1~NPP_value+Water_Index,umf)
mod.NPP <- occu(~1~NPP_value,umf)

mod.HFI.water <- occu(~1~HFI+Water_Index,umf)
mod.HFI <- occu(~1~HFI,umf)

mod.water <- occu(~1~Water_Index, umf)

#list models and compare AIC (including univariate models)
MultiList <- fitList(mod0,mod.NPP.HFI,mod.NPP.water, mod.NPP, mod.HFI.water, mod.HFI, mod.water)

#model selection
modSel(MultiList) #mod.HFI, mod.NPP.HFI and mod.HFI.water equally supported
...
summary(mod.NPP.HFI) 
summary(mod.NPP.water) 
summary(mod.NPP)
summary(mod.HFI.water) 
summary(mod.HFI)
summary(mod.water)
summary(mod0)
